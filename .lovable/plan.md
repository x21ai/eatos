# Rename Customer Facing Display to Guest Facing Display

Replace every user-facing mention of "Customer Facing Display", "Customer-Facing Display", "Customer Display", and the "CFD" abbreviation with "Guest Facing Display" across the whole site, and move the product page to a new URL with the old link preserved.

## What changes for visitors

- Navigation menus, footer, homepage, platform and products pages, brochures, media kit, support, shop, quick service and all demo/animation sections read "Guest Facing Display".
- The product page moves from `/products/customer-facing-display` to `/products/guest-facing-display`. Anyone visiting the old address, including existing backlinks, is permanently forwarded to the new one, so no traffic or ranking equity is lost.
...
- "Customer Facing Display" / "Customer-Facing Display" -> "Guest Facing Display"
- "Customer Display" -> "Guest Facing Display"
- Standalone "CFD" label shown to visitors -> "Guest Facing Display"
- Product short names and tab labels -> "Guest Facing Display"

Not changed: unrelated uses of the word "customer" (for example "customer loyalty", "customer support", "our customers").

## Technical notes

- Rename the route folder `src/app/products/customer-facing-display` to `src/app/products/guest-facing-display` and update its `content.ts`, `page.tsx` metadata and the client component name.
- Add a permanent (308) entry to the existing `redirects()` list in `next.config` mapping `/products/customer-facing-display` to `/products/guest-facing-display`.
...
- Verify afterwards: `/products/guest-facing-display` renders, the old path redirects, nav and footer links resolve, and no visitor-facing "CFD" or "Customer Display" strings remain.
