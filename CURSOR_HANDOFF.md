...
Keep the live Crisp chat launcher usable while the cookie consent bar is on screen.

## Recent changes

- 2026-09-25 — Lifted the Crisp launcher above the cookie consent bar: `CookieBanner` publishes its measured height (now re-measured with a `ResizeObserver`) and the Crisp widget overrides Crisp's launcher offset custom properties, clearing the override on consent.
...
- After deploy, confirm on live that the Crisp bubble clears the consent bar on desktop and mobile and drops back to its default position once consent is given.
